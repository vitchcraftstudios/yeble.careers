...
function serializeCandidate(candidate: any) {
  return {
    ...
    files: candidate.files.map((file: any) => ({
      id: file.id,
    ...
    applications: candidate.applications.map((application: any) => ({
      id: application.id,
    ...
    payments: candidate.payments.map((payment: any) => ({
      id: payment.id,
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error(error);
    return NextResponse.json({ error: "Unable to delete registrant profile" }, { status: 500 });
  }
}
